...
//!
//! Falcon operates over the polynomial ring R_q = Z_q[X]/(X^n + 1), where:
//! - q = 12289 is a prime (chosen so q ≡ 1 mod 2n, enabling NTT)
//! - n = 512 for Falcon-512
//! - X^n + 1 is the n-th cyclotomic polynomial
//!
//! The NTT transforms polynomials from coefficient representation to
//! evaluation representation, where polynomial multiplication becomes
//! pointwise multiplication (O(n) instead of O(n²)).
//!
//! # Montgomery Arithmetic
//!
//! All arithmetic uses Montgomery representation for efficient modular
//! reduction without division. A value `a` is stored as `aR mod q` where
//! R = 2^16. Montgomery multiplication computes `(aR * bR) / R mod q = abR mod q`.
//!
//! # References
//!
//! - Falcon specification: <https://falcon-sign.info/falcon.pdf>
//! - NTT explanation: Section 3.7 of the Falcon specification
//! - Montgomery multiplication: <https://en.wikipedia.org/wiki/Montgomery_modular_multiplication>

use crate::FALCON_512_N;

/// The prime modulus q = 12289.
///
/// This prime was chosen because:
/// 1. q ≡ 1 (mod 2048), which means primitive 2048-th roots of unity exist in Z_q
/// 2. q is small enough that products fit in 32 bits before reduction
/// 3. q is large enough to provide adequate security margin
const Q: u32 = 12289;

/// Montgomery reduction constant: -1/q mod 2^16 = 12287.
///
/// Used in Montgomery multiplication to compute the correction factor.
/// Derived from: Q0I * q ≡ -1 (mod 2^16)
/// Verification: 12287 * 12289 = 151,041,743 = 2305 * 65536 - 1 ≡ -1 (mod 65536)
const Q0I: u32 = 12287;

/// Montgomery radix: R = 2^16 mod q = 4091.
///
/// The Montgomery representation of 1 (i.e., 1 * R mod q).
/// All values in Montgomery form are multiplied by R.
const R: u32 = 4091;

/// Montgomery conversion constant: R² = 2^32 mod q = 10952.
///
/// Used to convert a standard integer to Montgomery form:
/// to_montgomery(a) = montymul(a, R²) = a * R² / R mod q = aR mod q
const R2: u32 = 10952;

/// Forward NTT twiddle factors in Montgomery form.
///
/// GMB[i] = g^(rev(i)) * R mod q, where:
/// - g = 7 is a primitive 2048-th root of unity modulo q (g^2048 ≡ 1 mod q)
/// - rev(i) is the bit-reversal of i over 10 bits
/// - R = 2^16 is the Montgomery radix
///
/// The bit-reversal ordering allows the iterative Cooley-Tukey NTT algorithm
/// to access elements in sequential order, improving cache performance.
///
/// Only the first 512 entries are used for Falcon-512 (n=512).
...
///
/// IGMB[i] = g^(-rev(i)) * R mod q, where:
/// - g^(-1) = 8778 mod q (the multiplicative inverse of g=7 modulo q)
/// - Verification: 7 * 8778 = 61446 = 5 * 12289 + 1 ≡ 1 (mod q)
///
/// These factors are used in the inverse NTT to convert from evaluation
/// (NTT domain) back to coefficient representation.
...
/// Computes (x + y) mod q using constant-time conditional subtraction.
#[inline(always)]
...
/// Computes (x - y) mod q using constant-time conditional addition.
#[inline(always)]
...
pub fn field_halve(x: u32) -> u32 {
    // Add q if x is odd: mask is Q if odd, 0 if even
    let x = x.wrapping_add(Q & (0u32.wrapping_sub(x & 1)));
    x >> 1
}

/// Montgomery multiplication: computes (x * y) / R mod q.
#[inline(always)]
pub fn montgomery_mul(x: u32, y: u32) -> u32 {
    let z = x * y;
    // Compute correction: w makes (z + w*q) divisible by 2^16
    let w = ((z.wrapping_mul(Q0I)) & 0xFFFF).wrapping_mul(Q);
    // Divide by 2^16 (the shift that makes Montgomery reduction efficient)
    let z = (z + w) >> 16;
    // Final reduction: if z >= q, subtract q
    let z = z.wrapping_sub(Q);
    z.wrapping_add(Q & (0u32.wrapping_sub(z >> 31)))
}

/// Forward NTT: transforms polynomial from coefficient to evaluation form (in-place, O(n log n)).
pub fn ntt_forward(a: &mut [u16; FALCON_512_N]) {
    let n = FALCON_512_N;
    let mut t = n;      // Distance between butterfly pairs
    let mut m = 1;      // Number of sub-transforms at current level

    // Iterate through log2(n) = 9 levels
    while m < n {
        let ht = t >> 1;    // Half of current distance
        let mut j1 = 0;     // Start index for current block

        for i in 0..m {
            // Twiddle factor for this butterfly group (in Montgomery form)
...
/// Inverse NTT: transforms polynomial from evaluation to coefficient form (in-place).
pub fn ntt_inverse(a: &mut [u16; FALCON_512_N]) {
    let n = FALCON_512_N;
    let logn = 9;       // log2(512) = 9
    let mut t = 1;      // Distance between butterfly pairs
    let mut m = n;      // Number of sub-transforms

    // Iterate through log2(n) = 9 levels (reverse order from forward NTT)
    while m > 1 {
        let hm = m >> 1;    // Half the number of transforms
        let dt = t << 1;    // Double the distance
        let mut j1 = 0;

        for i in 0..hm {
            let j2 = j1 + t;
            // Inverse twiddle factor (in Montgomery form)
    ...
    // Final step: divide all coefficients by n
    // Compute ni = R/n = R/512 in Montgomery form by halving R nine times
    let mut ni = R;
    for _ in 0..logn {
        ni = field_halve(ni);
    }
    // Multiply each coefficient by 1/n (via Montgomery multiplication with R/n)
    for i in 0..n {
        a[i] = montgomery_mul(a[i] as u32, ni) as u16;
    }
}

/// Converts a polynomial to Montgomery representation (a[i] -> a[i] * R mod q).
...
/// Pointwise multiplication in NTT domain. Both inputs must be in NTT domain and Montgomery form.
...
/// Computes f - g coefficient-wise, storing result in f.
///
/// Both polynomials must be in the same representation (either both
/// in coefficient form or both in NTT form).
...
/// Prepares a public key polynomial for verification.
///
/// Converts the public key h to NTT domain and Montgomery representation
/// in a single pass. This is the form needed for efficient polynomial
/// multiplication during signature verification.
