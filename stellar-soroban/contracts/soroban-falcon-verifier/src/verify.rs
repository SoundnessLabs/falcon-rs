...
use crate::ntt::{ntt_forward, ntt_inverse, poly_pointwise_mul, poly_sub, poly_prepare_for_mul};
use crate::{Q, FALCON_512_N, FALCON_512_PUBKEY_SIZE, L2_BOUND_512};
...
impl FalconVerifier {
    ...
    pub fn verify_raw_512(c0: &[u16; FALCON_512_N], s2: &[i16; FALCON_512_N], h: &[u16; FALCON_512_N]) -> bool {
    ...
    pub fn decode_pubkey(pubkey: &[u8], h: &mut [u16; FALCON_512_N]) -> bool {
        ...
        if pubkey[0] != 9 {  // logn = 9 for Falcon-512
            return false;
        }
    ...
    fn decode_sig_ct(data: &[u8], s2: &mut [i16; FALCON_512_N]) -> usize {
        ...
        while u < n {
            ...
            while acc_len >= BITS && u < n {
                ...
                // The value -2^(bits-1) = -2048 is forbidden
                // In 12-bit two's complement, -2048 is represented as 0x800
        ...
        // Any leftover bits must be zero
        if (acc & ((1u32 << acc_len) - 1)) != 0 {
            return 0;
        }
    ...
    fn hash_to_point(nonce: &[u8], message: &[u8], c0: &mut [u16; FALCON_512_N]) {
        ...
        // Initialize SHAKE256 extendable output function
        let mut hasher = Shake256::default();
        hasher.update(nonce);      // 40 bytes of randomness
        hasher.update(message);    // The actual message
        let mut xof = hasher.finalize_xof();

        // Generate 512 coefficients using rejection sampling
        let mut remaining = FALCON_512_N;
        let mut idx = 0;

        while remaining > 0 {
            // Extract 2 bytes (16 bits) from SHAKE256
            let mut buf = [0u8; 2];
            xof.read(&mut buf);

            // Interpret as big-endian u16
            let w = ((buf[0] as u32) << 8) | (buf[1] as u32);

            // Rejection sampling: accept only if w < 5*q = 61445
            const ACCEPT_THRESHOLD: u32 = 5 * 12289;  // 61445
            if w < ACCEPT_THRESHOLD {
                // Reduce w mod q using repeated subtraction
                // (faster than division for small multiples)
                let mut v = w;
                while v >= Q {
                    v -= Q;
                }
            ...
            // If w >= 61445, we silently reject and loop again
        }
    }
}
...
mod tests {
    ...
    fn test_cross_verify_with_c_bindings() {
        ...
        assert!(FalconVerifier::verify_512(&pubkey, b"Hello, Falcon!", &sig_0_bytes),
        ...
        assert!(FalconVerifier::verify_512(&pubkey, b"Test message for cross-implementation verification", &sig_1_bytes),
        ...
        let empty_msg: &[u8] = b"";
        assert!(FalconVerifier::verify_512(&pubkey, empty_msg, &sig_2_bytes),
        ...
        assert!(FalconVerifier::verify_512(&pubkey, &binary_msg, &sig_3_bytes),
                "falcon512_3: binary data verification failed");

        // Negative tests - wrong messages should fail
        assert!(!FalconVerifier::verify_512(&pubkey, b"Wrong message", &sig_0_bytes),
                "Wrong message should fail");
        assert!(!FalconVerifier::verify_512(&pubkey, b"Hello, Falcon!", &sig_1_bytes),
                "Mismatched signature should fail");
        assert!(!FalconVerifier::verify_512(&pubkey, b"Hello, Falcon!", &sig_2_bytes),
